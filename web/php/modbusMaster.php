 ...
 *  
 */
require_once __DIR__.'/IecType.php';
require_once __DIR__.'/PhpType.php'; 
/**
 * ModbusMaster
 *
...
class ModbusMaster {
  ...
  private function connect(){
    ...
    //	Liaison de l'adresse Ip du client et de la socket de communication
    if (strlen($this->client)>0){
        $result = socket_bind($this->sock, $this->client, $this->client_port);
        if ($result === false) {
            throw new Exception("Echec de liaison de socket.</br> : ($result)".
                socket_strerror(socket_last_error($this->sock)));
        } else {
            $this->status .= "Lié\n";	//	Bound
        }
    }
    // Définition du délai d'exécution pour les fonctions sortantes bloquantes
    socket_set_option($this->sock, SOL_SOCKET, SO_SNDTIMEO, array('sec' => 1, 'usec' => 0));

    // Création de la connexion sur la socket
    $result = @socket_connect($this->sock, $this->host, $this->port);
    if ($result === false) {
        throw new Exception("Echec de connexion socket</br> : ($result)".
            socket_strerror(socket_last_error($this->sock)));
    } else {
